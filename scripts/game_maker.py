...
if platform.system() == 'Linux':
	...
	def find_archive():
		path = pjoin(os.getenv("HOME"), "Library/Application Support/Steam/SteamApps/common/CookServeDelicious3/Cook Serve Delicious 3.app/Contents/Resources/game.ios")
		if isfile(path):
			return path

		path = "/Applications/Cook Serve Delicious 2.app/Contents/Resources/game.ios"
		if isfile(path):
			return path
		...
		for key, subkey, valkey in reg_keys:
			try:
				...
				try:
					...
					if valtype == REG_SZ:
						return pjoin(value, "steamapps\\common\\CookServeDelicious3\\data.win")
				finally:
					CloseKey(hkey)
			except FileNotFoundError:
				pass
...
def find_archive_wine():
	# TODO: parse wine registry files instead of guessing
	linux_paths = [
		[".wine", "drive_c", "Program Files (x86)", "Steam", "steamapps", "common", "CookServeDelicious3", "data.win"],
		[".wine", "drive_c", "Program Files", "Steam", "steamapps", "common", "CookServeDelicious3", "data.win"],
	]
