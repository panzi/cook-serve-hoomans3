CC=gcc
BINNAME=cook_serve_hoomans3
# stupid github limits file names unnecesarrily
WIN_BINNAME=Cook-Serve-Hoomans-3
BINEXT=
...
CSH3_OBJ=$(BUILDDIR_BIN)/cook_serve_hoomans3.o \
         ...
         $(BUILDDIR_BIN)/csh3_patch_def.o

CSH3_DATA_OBJ=$(patsubst $(BUILDDIR_SRC)/%.c,$(BUILDDIR_BIN)/%.o,$(wildcard $(BUILDDIR_SRC)/csh3_*_data.c))
...
ifeq ($(TARGET),win32)
	...
	ARCH_FLAGS=-m32
	BINEXT=.exe
	CSH3_OBJ+=$(BUILDDIR_BIN)/resources.o
	BINNAME=$(WIN_BINNAME)
else
ifeq ($(TARGET),win64)
...
all: cook_serve_hoomans3 gmdump gmupdate gminfo

cook_serve_hoomans3: "$(BUILDDIR_BIN)/$(BINNAME)$(BINEXT)"
...
macpkg: $(BUILDDIR_BIN)/cook_serve_hoomans3_$(VERSION)_mac.zip

icon: $(BUILDDIR_SRC)/icon.ico

$(BUILDDIR_BIN)/cook_serve_hoomans3_$(VERSION)_mac.zip: \
		$(BUILDDIR_BIN)/cook_serve_hoomans3 \
		$(BUILDDIR_BIN)/cook_serve_hoomans3.command \
		$(BUILDDIR_BIN)/open_with_cook_serve_hoomans3.command \
		$(BUILDDIR_BIN)/README.txt
	mkdir -p $(BUILDDIR_BIN)/bin
	cp $(BUILDDIR_BIN)/cook_serve_hoomans3 $(BUILDDIR_BIN)/bin
	cd $(BUILDDIR_BIN); zip -r9 cook_serve_hoomans3_$(VERSION)_mac.zip \
		bin \
		cook_serve_hoomans3.command \
		open_with_cook_serve_hoomans3.command \
		README.txt
...
$(BUILDDIR_BIN)/csh3_patch_def.o: $(BUILDDIR_SRC)/csh3_patch_def.c src/game_maker.h
...
$(BUILDDIR_BIN)/cook_serve_hoomans3.o: \
		src/cook_serve_hoomans3.c \
		$(BUILDDIR_SRC)/csh3_patch_def.h
	$(CC) $(ARCH_FLAGS) $(CFLAGS) -c $< -o $@

# recursive make trick so $(CSH3_DATA_OBJ) is expanded for the generated C files
internal_make_binary: $(CSH3_OBJ) $(CSH3_DATA_OBJ)
	$(CC) $(ARCH_FLAGS) $(CFLAGS) $(CSH3_OBJ) $(CSH3_DATA_OBJ) -o "$(BUILDDIR_BIN)/$(BINNAME)$(BINEXT)"

"$(BUILDDIR_BIN)/$(BINNAME)$(BINEXT)": $(BUILDDIR_SRC)/csh3_patch_def.h $(CSH3_OBJ)
...
$(ICONS): icon/raccoon.svg
	$(INKSCAPE) $< --export-width=$(patsubst $(BUILDDIR_SRC)/icon_%.png,%,$@) \
		--export-area-page --export-overwrite --export-type=png --export-filename=$@
...
clean:
	rm -f \
		...
		$(CSH3_OBJ) \
		$(CSH3_DATA_OBJ) \
		$(BUILDDIR_SRC)/csh3_*_data.c \
		$(BUILDDIR_SRC)/csh3_patch_def.h \
		$(BUILDDIR_SRC)/csh3_patch_def.c \
